...
public class InventoryUI : MonoBehaviour
{
    ...
    public void DecreaseGoodsCount(GoodsType goodsType)
    {
        ...
        int currentCount = Inventory_KJS.instance.GetGoodsCount(goodsType);
    ...
    public void ActivateDisabledObject(GoodsType goodsType)
    {
        ...
        for (int i = 0; i < disabledObjects.Count; i++)
        {
            ...
            if (goodsInfo != null && goodsInfo.goodsType == goodsType)
            {
                ...
                Debug.Log($"{obj.name} 오브젝트가 활성화되었습니다. Type: {goodsType}");
    ...
    public void UpdateSpecificText(GoodsType goodsType)
    {
        ...
        if (index >= 0 && index < itemCountTexts.Length)
        {
            ...
            int count = Inventory_KJS.instance.GetGoodsCount(goodsType);
        ...
        {
            Debug.LogWarning($"해당 GoodsType에 대응하는 TextMeshPro가 없습니다: {goodsType}");
        }
    }
}
